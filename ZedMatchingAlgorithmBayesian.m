...
for time_seg = 1 : len
    ...
    x4 = cellfun(@(v)v(time_seg,4),Adindiv(:,asscol));
    x5 = cellfun(@(v)v(time_seg,5),Adindiv(:,asscol));
    
    Frames_FrameMinDist{time_seg,2} = x1;
    Frames_FrameMinDist{time_seg,3} = x2;
    Frames_FrameMinDist{time_seg,4} = x3;
    Frames_FrameMinDist{time_seg,5} = x4;
    Frames_FrameMinDist{time_seg,6} = x5;
%     MaxVar = max([var(x1),var(x2),var(x3)]);
%     Frames_FrameMinDist{time_seg,6} = [var(x1,2),var(x2,2),var(x3,2)];%,x1./max(x1(:)),x2./max(x2(:)),x3./max(x3(:))];
    %%
