...
% Cite: Yi Cao (2020). Munkres Assignment Algorithm (https://www.mathworks.com/matlabcentral/fileexchange/20328-munkres-assignment-algorithm), MATLAB Central File Exchange. Retrieved November 17, 2020.
...
%**************************************************************************
zP = ~dMat; % rows and columns have zero value
starZ = false(n); % star zeros
while any(zP(:))
    [r,c]=find(zP,1); % returns first row and column that has zero 
    starZ(r,c)=true;
    zP(r,:)=false; % do not go through that row again
    zP(:,c)=false; % do not go through that column again
end
...
%**************************************************************************
    ...
    while 1
        ...
        if Step == 6
            ...
            % each covered  % similar to step 5a in the video: https://www.youtube.com/watch?v=SAPG2T4Jbok
            %         row, and subtract it from every element of each uncovered column.
            %         Return to Step 4 without altering any stars, primes, or covered lines.
            %**************************************************************************
            M=dMat(~coverRow,~coverColumn);
            minval=min(min(M)); % find min uncovered value
            if minval==inf
                return
            end
            dMat(coverRow,coverColumn)=dMat(coverRow,coverColumn)+minval; % add min uncovered value to values covered by 2 lines
            dMat(~coverRow,~coverColumn)=M-minval; % subtract the min value from all uncovered values
        else
            break
        end
    end
